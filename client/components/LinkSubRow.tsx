...
import { useRemoveLinkFromLinksArrayMutation, useVoteUpMutation } from 'generated/graphql';
...
const LinkSubRow = ({votes, postedBy, hoursAgo, linksArray, id}) => {
  ...
  const handleClick = async (event) => {
    ...
    try {
      ...
      const res = await removeLinkFromLinksArray({variables: {id, linkId, email}});
  ...
  const voteDownCall = async ({id: linkId}:number) =>{
    try {
